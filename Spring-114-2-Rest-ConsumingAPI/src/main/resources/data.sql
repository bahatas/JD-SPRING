...
INSERT INTO ticket (date_time, row_number, seat_number, movie_cinema_id, user_account_id) VALUES
                   ...
                   ('2022-05-02 20:00:00', 18, 7, (SELECT id FROM movie_cinema WHERE cinema_id = (SELECT c.id FROM cinema c WHERE c.name = 'Hall 1 - EMPIRE') AND movie_id = (SELECT m.id FROM movie m WHERE m.name = 'The Gentleman') LIMIT 1), (SELECT id from user_account WHERE email = 'bernard@email.com')),
